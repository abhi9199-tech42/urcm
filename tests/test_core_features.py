...
class TestSafetyGovernor:
    ...
    def test_kernel_unlock_with_key(self, monkeypatch):
        from urcm.config import reload_settings
        monkeypatch.setenv("URCM_ADMIN_KEY", "test-admin-key")
        reload_settings()
        gov = SafetyGovernor()
        gov.lock_kernel()
        gov.unlock_kernel("test-admin-key")
        gov.validate_modification("core_logic_rewrite")  # no raise

    def test_kernel_unlock_bad_key(self, monkeypatch):
        from urcm.config import reload_settings
...
class TestReversibility:
    ...
    def test_decoder_trajectory_shapes(self):
        ...
        assert states.shape[0] <= 5 and states.shape[1] == 64
        assert outputs.shape[0] <= 5 and outputs.shape[1] == 24

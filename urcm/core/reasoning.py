...
class ReasoningEngine:
    ...
    def __init__(self, brain_path: str = "urcm_identity.pkl"):
        ...
        if os.path.exists(brain_path):
            ...
            logging.getLogger(__name__).info(
                "Brain file %s not found — creating fresh brain with random weights.", brain_path
            )
            self.brain_data = self._create_fresh_brain()
            ...
            if self.concept_map[first_key].shape[0] != self.l2_dim:
                 ...
                 if os.path.exists("urcm_weights.pkl"):
                     ...
                     if "l2_W_res" in weights and weights["l2_W_res"].shape[0] == self.l2_dim:
                         ...
                         print(f"Re-initialized W_res to {self.l2_dim}x{self.l2_dim} (Random)")
                 else:
                     self.hierarchy.layer2.W_res = np.random.randn(self.l2_dim, self.l2_dim) * 0.01
                     print(f"Re-initialized W_res to {self.l2_dim}x{self.l2_dim} (Random - No weights file)")
    ...
    @staticmethod
    def _create_fresh_brain(l2_dim: int = 512) -> dict:
        """Create a minimal fresh brain with random weights and empty concept map."""
        return {
            "l2_W_res": np.random.randn(l2_dim, l2_dim) * 0.01,
            "l2_W_in": np.random.randn(l2_dim, l2_dim) * 0.01,
            "l2_W_out": np.random.randn(l2_dim, l2_dim) * 0.01,
            "concept_map": {},
            "relations": [],
            "type_map": {},
            "numeric_constraints": [],
    ...
    def solve(self,
        ...
        Args:
            ...
            constraints: List of (concept_name, weight) tuples for constraint-based reasoning
            logic_gates: Optional list of logic gate dictionaries
            steps: Number of reasoning steps

        Returns:
            List of words/concepts traversed during reasoning
        """
        if logic_gates is None:
            logic_gates = []

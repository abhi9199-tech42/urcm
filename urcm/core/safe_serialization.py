...
class _RestrictedUnpickler(pickle.Unpickler):
    """Unpickler that only allows safe types from ALLOWED_PICKLE_TYPES."""
    def find_class(self, module: str, name: str) -> Any:
        # Allow numpy array reconstruction
        if module == 'numpy._core.multiarray' and name in ('_reconstruct', 'ndarray'):
            return super().find_class(module, name)
        if module == 'numpy' and name in ('ndarray', 'dtype', 'float64', 'float32', 'float16',
                                           'int64', 'int32', 'int16', 'int8',
                                           'uint64', 'uint32', 'uint16', 'uint8'):
            return super().find_class(module, name)
        # Allow builtins for basic types
        if module == 'builtins' and name in ('dict', 'list', 'tuple', 'set', 'str',
                                              'int', 'float', 'bool', 'bytes', 'NoneType'):
            return super().find_class(module, name)

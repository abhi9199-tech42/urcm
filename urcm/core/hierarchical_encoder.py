    ...
    def _load_weights_if_exist(self):
        ...
        if os.path.exists(weight_path):
            try:
                ...
                if "l1_W_out" in weights: self.layer1.W_out = weights["l1_W_out"]
                if "l1_bias" in weights: self.layer1.bias = weights["l1_bias"]
                ...
                if "l2_W_res" in weights:
                    ...
                    if saved_dim == self.l2_res_dim:
                        ...
                        if "l2_W_out" in weights: self.layer2.W_out = weights["l2_W_out"]
                        if "l2_bias" in weights: self.layer2.bias = weights["l2_bias"]
                        print(f"Loading trained weights from {weight_path}...")
                    else:
                        print("⚠️ Weight dimension mismatch. Using random initialization.")
